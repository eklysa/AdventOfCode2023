# -*- coding: utf-8 -*-
"""
Created on Thu Jan 18 19:20:46 2024
...
for symbolRow in symbolLocations: 
    for symbol in symbolRow:
        ...
        digits_per_symbol.append(adjacent_digits)

gears = []
for symbol in digits_per_symbol: 
    if len(symbol) == 2: 
        gears.append(symbol)
...
for gear in gears: 
    ...
    for number in gear:
        ...
        for location in number: 
            ...
            digits.append(digit)
        digits2.append(digits)
    sum_digits.append(digits2)

number_list = []
for symbol in sum_digits:     
    number_list.append([int(''.join(sublist)) for sublist in symbol if sublist])
    
ratios = []
for symbol in number_list: 
    ratio = symbol[0] * symbol[1]
    ratios.append(ratio)

print('ratio: ', sum(ratios))
